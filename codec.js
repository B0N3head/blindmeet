export function encodeDates(dates) {
...
export function decodeDates(encoded) {
  const dates = [];
...
export function encodeAvailability(slots, startHour, endHour) {
...
export function decodeAvailability(encoded, startHour, endHour) {
  const numSlots = (endHour - startHour) * 2;
  const slots = [];
  for (const entry of encoded) {
